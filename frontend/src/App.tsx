...
const App: React.FC = () => {
  ...
  const fetchS3Objects = async (prefix: string, token?: string) => {
    try {
      ...
      console.log("fetchS3Objects: Sending request with prefix:", prefix, "and token:", token);
      const response = await axios.get<FileResponse>(
        `http://localhost:3000/api/s3/list?prefix=${prefix}${
      ...
      console.log("fetchS3Objects: Received nextContinuationToken:", response.data.nextContinuationToken);
      setLogs((prev) => ({
        ...prev,
        status: "S3 objects listed successfully.",
      }));
    } catch (error) {
      console.error("fetchS3Objects: Error fetching S3 objects:", error);
      setLogs((prev) => ({
        ...prev,
        status: "failed to run",
        errors: [
  ...
  const handleDeleteS3File = async (key: string) => {
    if (window.confirm(`Are you sure you want to delete ${key}?`)) {
      try {
        ...
        const response = await axios.post(
          "http://localhost:3000/api/s3/delete",
          { keys: [key] }
        );
        if (response.data.statusCode !== 200) {
        ...
        setLogs((prev) => ({
          ...prev,
          status: `Successfully deleted ${key}.`,
        }));
  ...
  const handleSearch = async () => {
    ...
    if (
      window.confirm(
        "This search may take a while for large directories. Continue?"
      )
    ) {
      try {
        ...
        const response = await axios.get<FileResponse>(
          `http://localhost:3000/api/s3/search?prefix=${currentPrefix}&transactionPattern=${encodeURIComponent(
            transactionNumberPattern
          )}&filenamePattern=${encodeURIComponent(filenamePattern)}`
        );
        if (response.data.statusCode !== 200) {
