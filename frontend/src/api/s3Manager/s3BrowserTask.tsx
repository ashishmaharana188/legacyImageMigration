...
const fetchS3Objects = async ({
  ...
  const { data } = await axios.get("http://localhost:3000/s3-list-objects", {
    params: {
      ...
      maxKeys: 10, // Limit to 10 items per page
    },
  });
  return data;
};
...
const S3BrowserTask: React.FC<S3BrowserTaskProps> = ({
  ...
  // [FIX] Added missing loading states
  ...
  const handleUploadToS3 = useCallback(
    async (localDir: string, prefix: string) => {
      // [FIX] Clear logs and set loading state
      ...
      try {
        await configPromise;
        // Pass localDir if provided, otherwise backend uses default
        const payload = localDir ? { localDir } : {};
    ...
    [updateTaskLog, queryClient, clearTaskLog]
  );
