...
import { SplitResult, splitVerification } from "./splitProcessorTypes";
...
export class Splitting {
  ...
  private async getTotalExpectedPagesFromCsv(): Promise<number> {
    ...
    try {
      ...
      const records: splitVerification[] = parse(csvContent, {
        columns: true,
  ...
  async splitFiles(): Promise<SplitResult> {
    ...
    const latestCsvPath = await this.getLatestProcessedCsvPath();
    let csvRecords: splitVerification[] = [];
    if (latestCsvPath) {
      try {
    ...
    const verificationLog: splitVerification[] = [];
